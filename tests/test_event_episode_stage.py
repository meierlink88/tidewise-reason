...
from ingestion.episcode.event.stages.episode import (
    EVENT_ENTITY_TYPE_NAMES,
    EXTRACTION_INSTRUCTIONS,
    GraphitiEpisodeStage,
    event_entity_types,
)
...
class EventEpisodeStageTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
    ...
    async def test_delegates_formal_event_to_native_add_episode(self) -> None:
        ...
        await GraphitiEpisodeStage(graphiti).execute(event())
    ...
    async def test_marks_native_episode_as_formal_event(self) -> None:
        driver = Driver()
        graphiti = SimpleNamespace(driver=driver, add_episode=AsyncMock())
        projector = GraphitiEpisodeStage(graphiti)
    ...
    async def test_replay_with_same_content_is_idempotent(self) -> None:
        ...
        self.assertEqual(len(driver.metadata_writes), 2)
        self.assertEqual(driver.existing[0]["episode_kind"], "EVENT")
        self.assertEqual(driver.existing[0]["domain_object_id"], EVENT_ID)
    ...
    async def test_pending_native_shell_is_resumed(self) -> None:
        ...
        await GraphitiEpisodeStage(graphiti).execute(historical)
    ...
    async def test_readiness_checks_graph_provider(self) -> None:
        ...
        self.assertTrue(await GraphitiEpisodeStage(graphiti).ready())
