"""Command-line boundary for the independent Concept projection."""
...
from projection.concept import build_plan, execute_plan, inspect_graph_state, load_facts, verify_state
from projection.runtime import ProjectionError, create_graphiti, load_config


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Project Concept facts into Graphiti")
    parser.add_argument("--env-file", type=Path, help="private runtime environment")
    commands = parser.add_subparsers(dest="command", required=True)
    commands.add_parser("plan", help="validate all paginated Concept facts")
    run = commands.add_parser("run", help="execute deterministic Namespace bulk writes")
    run.add_argument(
        "--replace",
        action="store_true",
        help="remove stale Concept facts owned by this projection before upsert",
    )
    commands.add_parser("verify", help="compare graph facts with the complete Concept API")
    return parser
...
async def _main(args: argparse.Namespace) -> dict[str, object]:
    ...
    if args.command == "plan":
        return {
            **plan.summary(),
            "first_concept": plan.nodes[0].name if plan.nodes else None,
